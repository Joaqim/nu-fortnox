use std assert
use ../../fortnox/resources/utils/fortnox_resource_url.nu

export def "Create resource url for Fortnox API" [] {
    assert equal (
        fortnox_resource_url invoices
...
export def "Create GET resource url with params for Fortnox API" [] {
    assert equal (
        fortnox_resource_url invoices --page 1 {
            limit: 100
            sortby: invoicedate
...
export def "Create POST resource url for invoices in Fortnox API" [] {
    assert equal (
        fortnox_resource_url invoices --additional-path bookkeep {
        } | url parse | select path params
    ) ({
...
export def "Create POST and GET resource urls for vouchers in series S from Fortnox API" [] {
    assert equal (
        fortnox_resource_url vouchers --additional-path S {
        } | url parse | select path params
    ) ({
        path: '/3/vouchers/S/'
        params: { }
    ...
    assert equal (
        fortnox_resource_url vouchers --id 1001 --additional-path S {
        } | url parse | select path params
    ) ({
